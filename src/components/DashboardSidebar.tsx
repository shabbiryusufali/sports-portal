...
export default function DashboardSidebar({ displayName, email, initials, isAdmin, signOutAction }: Props) {
  ...
  return (
    <>
      ...
      <header className="sp-topbar">
        <Link href="/dashboard" className="text-[15px] font-black tracking-tighter" style={{ letterSpacing: "-0.04em" }}>
          SPORTS<span style={{ color: "var(--accent)" }}>PORTAL</span>
        </Link>
        <button
          onClick={() => setOpen(v => !v)}
          aria-label="Toggle menu"
          style={{ color: "var(--text-secondary)", padding: "6px", borderRadius: "8px", background: "transparent", border: "none", cursor: "pointer" }}
        >
      ...
      {/* ── Mobile overlay ───────────────────────────────────────────── */}
      <div className={`sp-overlay ${open ? "open" : ""}`} style={{ display: open ? "block" : "none" }} onClick={() => setOpen(false)} />
      ...
      <aside className={`sp-sidebar ${open ? "open" : ""}`}>
        ...
        <div style={{ padding: "24px 20px 20px", borderBottom: "1px solid var(--border)" }}>
          <Link
            href="/dashboard"
            style={{ fontSize: "18px", fontWeight: 900, letterSpacing: "-0.045em", textDecoration: "none", color: "var(--text-primary)", display: "block" }}
          >
            SPORTS<span style={{ color: "var(--accent)" }}>PORTAL</span>
          </Link>
          <p style={{ fontSize: "0.7rem", color: "var(--text-muted)", marginTop: "4px", fontWeight: 500 }}>Sports Management Platform</p>
        </div>
        ...
        <nav style={{ flex: 1, padding: "14px 12px", display: "flex", flexDirection: "column", gap: "2px" }}>
          ...
          {isAdmin && (
            <>
              ...
              <Link href="/dashboard/admin" className={`sp-nav-item ${isActive("/dashboard/admin") ? "active" : ""}`} style={{ color: isActive("/dashboard/admin") ? "#fbbf24" : undefined }}>
                <SettingsIcon />
                Admin Panel
              </Link>
            </>
        ...
        <div style={{ padding: "12px", borderTop: "1px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", borderRadius: "12px", background: "rgba(255,255,255,0.03)", marginBottom: "8px" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: "0.8125rem", fontWeight: 600, color: "var(--text-primary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{displayName}</p>
              <p style={{ fontSize: "0.6875rem", color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{email}</p>
            </div>
          </div>
