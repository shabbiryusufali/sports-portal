...
export default async function ProfilePage() {
      ...
      player: {
        include: {
          ...
          matchesAsPlayerA: {
            ...
            select: { score_team_a: true, score_team_b: true, match_date: true, event: { select: { name: true } } },
          },
          matchesAsPlayerB: {
            where: { status: "COMPLETED" },
            select: { score_team_a: true, score_team_b: true, match_date: true, event: { select: { name: true } } },
  ...
  const resultConfig = {
    W: { label: "W", bg: "rgba(0,255,135,0.1)", color: "#00ff87", border: "rgba(0,255,135,0.2)" },
    L: { label: "L", bg: "rgba(248,113,113,0.1)", color: "#f87171", border: "rgba(248,113,113,0.2)" },
    D: { label: "D", bg: "rgba(255,255,255,0.05)", color: "#a1a1aa", border: "rgba(255,255,255,0.1)" },
  } as const;
  ...
  return (
    <div style={{ padding: "32px 32px 40px", maxWidth: 900, width: "100%" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 32 }}>
        ...
        <div>
          <h1 className="sp-page-title" style={{ fontSize: "1.625rem" }}>{user.name ?? user.username}</h1>
          <p style={{ fontSize: "0.875rem", color: "var(--text-secondary)", marginTop: 4 }}>@{user.username} · {user.email}</p>
          {user.player && (
      ...
      {user.player && allMatches.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
            {[
              { label: "Matches",  value: allMatches.length, color: "var(--text-primary)" },
              { label: "Wins",     value: totalWins,         color: "var(--accent)" },
              { label: "Losses",   value: totalLosses,       color: "#f87171" },
              { label: "Win Rate", value: `${winRate}%`,     color: winRate >= 50 ? "var(--accent)" : "var(--text-secondary)" },
            ].map((s) => (
              <div key={s.label} className="sp-stat-card" style={{ textAlign: "center", padding: "16px 12px" }}>
          ...
          <div className="sp-card" style={{ padding: "16px 20px" }}>
            <p className="sp-section-title" style={{ marginBottom: 12 }}>Recent Form</p>
            <div style={{ display: "flex", gap: 8 }}>
              {recentForm.map((m, i) => {
                const rc = resultConfig[m.result as keyof typeof resultConfig];
                return (
                  <div key={i} style={{ width: 38, height: 38, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.75rem", fontWeight: 800, background: rc.bg, color: rc.color, border: `1px solid ${rc.border}` }}>
                    {rc.label}
                  </div>
                );
              })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20 }}>
        ...
        <div className="sp-card" style={{ padding: "24px" }}>
          <p className="sp-section-title" style={{ marginBottom: 18 }}>Account</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              ...
              <div key={row.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                ...
                <span style={{ fontSize: "0.8125rem", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "200px" }}>{row.value}</span>
              </div>
            ))}
          </div>
        </div>
