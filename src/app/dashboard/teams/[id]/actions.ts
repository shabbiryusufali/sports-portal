...
export async function getTeamDetail(teamId: string) {
  ...
  const [team, currentUser] = await Promise.all([
    prisma.team.findUnique({
      ...
      include: {
        sport: true,
        captain: { include: { user: { select: { id: true, name: true, email: true } } } },
        members: { include: { user: { select: { id: true, name: true, email: true } } } },
        events: { orderBy: { start_time: "desc" }, take: 10, include: { sport: true } },
        matchesAsTeamA: {
  ...
  const allMatches = [
    ...team.matchesAsTeamA.map((m) => ({
      id: m.id, date: m.match_date, opponent: m.team_b,
      scoreFor: m.score_team_a, scoreAgainst: m.score_team_b,
      result: m.score_team_a > m.score_team_b ? "W" : m.score_team_a < m.score_team_b ? "L" : "D",
      event: m.event,
    })),
    ...team.matchesAsTeamB.map((m) => ({
      id: m.id, date: m.match_date, opponent: m.team_a,
      scoreFor: m.score_team_b, scoreAgainst: m.score_team_a,
...
export async function transferCaptaincy(
  ...
  try {
    ...
    if (!isCaptain && !isAdmin)
      return { success: false, message: "Only the captain or an admin can transfer captaincy." };

    const isNewCaptainMember = team.members.some((m) => m.id === newCaptainPlayerId);
    if (!isNewCaptainMember)
      return { success: false, message: "The new captain must already be a team member." };
