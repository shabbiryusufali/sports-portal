...
type AdminEvent = {
  ...
  _count: { matches: number; participants: number };
};
type AdminTeam = {
  id: string;
  name: string;
...
function Notice({ type, msg }: { type: "ok" | "err"; msg: string }) {
  return (
    <div
      className={`sp-notice ${type === "ok" ? "sp-notice-ok" : "sp-notice-err"}`}
    ...
    <div>
      ...
      <div className="sp-card" style={{ overflow: "hidden" }}>
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "1fr 140px 90px 90px 130px",
            padding: "10px 20px",
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    act(
                      () => deleteUser(u.id),
                      () =>
                        setLocalUsers((p) => p.filter((x) => x.id !== u.id)),
                    );
                  }}
    ...
    <div>
      ...
      <div className="sp-card" style={{ overflow: "hidden" }}>
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "1fr 100px 90px 180px",
            padding: "10px 20px",
            borderBottom: "1px solid var(--border)",
            fontSize: "0.68rem",
            fontWeight: 700,
            ...
            >
              <div style={{ minWidth: 0 }}>
                ...
                >
                  ...
                  {e._count.participants} teams · {e._count.matches} matches ·
                  by {e.organizer.name ?? e.organizer.email}
                </p>
              </div>
              <span
                style={{ fontSize: "0.78rem", color: "var(--text-secondary)" }}
                  ...
                  onChange={(ev) => {
                    ...
                    start(async () => {
                      ...
                      if (res.success)
                        setLocalEvents((p) =>
                          p.map((x) =>
                            x.id === e.id ? { ...x, status: newStatus } : x,
                          ),
                        );
              ...
              >
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      !confirm(`Delete event "${e.name}" and all its matches?`)
                    )
                      return;
                    start(async () => {
                      const res = await deleteEvent(e.id);
                      if (res.success)
                        setLocalEvents((p) => p.filter((x) => x.id !== e.id));
                      else flash("err", res.message ?? "Failed");
                    });
                  }}
                  className="sp-btn-danger"
    ...
    <div>
      ...
      <div className="sp-card" style={{ overflow: "hidden" }}>
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "1fr 100px 80px 150px",
            padding: "10px 20px",
            borderBottom: "1px solid var(--border)",
            fontSize: "0.68rem",
            fontWeight: 700,
          ...
          filtered.map((t) => (
            ...
            >
              <div style={{ minWidth: 0 }}>
                ...
                >
                  {t._count.members} members · {t._count.events} events · Cap:{" "}
                  {t.captain?.user.name ?? t.captain?.user.email ?? "—"}
                </p>
              </div>
              <span
                style={{ fontSize: "0.78rem", color: "var(--text-secondary)" }}
              ...
              <span
                className={`badge ${t.is_active ? "badge-green" : "badge-red"}`}
              >
...
function MatchesTab({ matches }: { matches: AdminMatch[] }) {
  ...
  function participantLabel(m: AdminMatch) {
    if (m.team_a || m.team_b)
      return `${m.team_a?.name ?? "?"} vs ${m.team_b?.name ?? "?"}`;
    if (m.player_a || m.player_b)
      return `${m.player_a?.first_name ?? "?"} vs ${m.player_b?.first_name ?? "?"}`;
    return "Unknown";
  }
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    <span
                      className={`badge ${STATUS_BADGE[m.status] ?? "badge-zinc"}`}
                    >
                      {m.status}
                    </span>
                  </div>
          ...
          >
            {editId === s.id ? (
              <form
                action={(fd) => {
                  start(async () => {
                    const res = await updateSport(s.id, fd);
                    if (res.success) {
                      setEditId(null);
                      flash("ok", "Updated!");
                    } else flash("err", res.message ?? "Failed");
              ...
              >
                ...
                >
                  <input
                    ...
                    type="checkbox"
                    value="true"
                    defaultChecked={s.is_team_sport}
                    style={{
                      width: 16,
                      height: 16,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    <span
                      className={`badge ${s.is_team_sport ? "badge-blue" : "badge-purple"}`}
                    >
                      {s.is_team_sport ? "Team" : "Individual"}
                    </span>
                  </div>
                  {s.description && (
                    <p
                  ...
                  >
                    {s._count.teams} teams · {s._count.events} events
                  </p>
                </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      start(async () => {
                        ...
                        if (res.success)
                          setSports((p) => p.filter((x) => x.id !== s.id));
                        else flash("err", res.message ?? "Failed");
                      });
                    }}
                    className="sp-btn-danger"
