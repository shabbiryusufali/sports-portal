...
export default function LeaderboardsClient({
  ...
  const [sortBy, setSortBy] = useState<
    "points" | "winRate" | "goalsFor" | "played"
  >("points");
  ...
  const filteredSports = useMemo(
    () =>
      sports.filter((s) =>
        mode === "teams" ? s.is_team_sport : !s.is_team_sport,
      ),
    [sports, mode],
  );
  ...
  const filteredTeams = useMemo(
    () =>
      teams
        .filter(
          (t) =>
            (sportFilter === "all" || t.sport.id === sportFilter) &&
            t.played > 0,
        )
        .sort((a, b) =>
          sortBy === "points"
  ...
  const filteredPlayers = useMemo(
    () =>
      players
        .filter(
          (p) =>
            (sportFilter === "all" || p.sport.id === sportFilter) &&
            p.played > 0,
        )
        .sort((a, b) =>
          sortBy === "points"
            ? b.points - a.points || b.scoreDiff - a.scoreDiff
            : sortBy === "winRate"
              ? b.winRate - a.winRate
              : sortBy === "goalsFor"
  ...
  const teamGroups = useMemo(() => {
    ...
    filteredTeams.forEach((t) => {
      if (!g[t.sport.name]) g[t.sport.name] = [];
      g[t.sport.name].push(t);
    });
  ...
  const playerGroups = useMemo(() => {
    ...
    filteredPlayers.forEach((p) => {
      if (!g[p.sport.name]) g[p.sport.name] = [];
      g[p.sport.name].push(p);
    });
    return Object.keys(g).length ? g : null;
    ...
    return (
      <div className="sp-card" style={{ overflow: "hidden" }}>
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "2rem 1fr repeat(6, 3.5rem)",
            padding: "10px 20px",
            ...
            >
              <div
                style={{
                  ...
                  background: isTop3 ? "rgba(255,255,255,0.015)" : undefined,
                ...
                onMouseLeave={(el) => {
                  (el.currentTarget as HTMLElement).style.background = isTop3
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: "0.7rem", color: "var(--text-muted)" }}>
                    {team.sport.name} · {team.memberCount}m
                  </p>
                </div>
                <span
                  style={
      ...
      >
          ...
          {(["teams", "players"] as const).map((m) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                background:
                  mode === m ? "rgba(255,255,255,0.08)" : "transparent",
                color:
        ...
        >
          ...
          <option value="goalsFor">
            {mode === "teams" ? "Sort: Goals Scored" : "Sort: Points Scored"}
          </option>
